...
export class AddProductComponent implements OnInit {
  ...
  addNow(){
    ...
    this.http.postRequest("/product/add", this.productPayload).subscribe(data=>{
      if(this.file != null){
        this.productPayload.urlImg = "images/products/"+this.productPayload.category_id+"/"+data.id+"/"+this.fileName
        this.productPayload.id = data.id;
        this.http.pushFileToStorage("/product/post-img?cateId="+this.productPayload.category_id+"&id="+data.id, this.file).subscribe(()=>{
